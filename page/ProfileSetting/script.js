...
function createInitDataTable(){
    let settingContents = `Enter number of rows to be added when adding rows is selected.
    Select whether to use "Receiving Date" as Stock Key.
    Enter default value of country code.
    Select encode type of CSV.
    Select whether to output header of CSV.
    Select line feed code of CSV.
    Enter data delimiter of CSV
    When updating the Customer Master, select whether to use the function to update the Ship-to ID = default value with the same contents as the Customer.
    Select whether to use "Expiration Date" as Stock Key.
    Select whether to use Quality Category as stock key. Receiving/stock is always valid regardless of this setting.
    Please select whether to display the Receiving Location in HT Quick Receiving.
    Please select whether to display the Receiving Location in HT Return Shipment.
    Trim space for the csv files
    Select whether to use import re-edit function.
    Select whether to set JAN to automatic numbering
    Enter 20 to 29 for first JAN character
    Select whether to use "Lot" as Stock Key
    Comparison range when setting the Order Point (Lower Limit)
    Please specify the number of days to turn off the transmission flag of order data from the date and time when it turns on. (1 to 999)
    In order to use restocking function, please choose an item from the below product master to register an upper limit of stock at picking location.
    Specify number of retention day of personal information. "0" not hidden.
    Select personal information deletion type
    Enter digits of decimal places of calculation result (retail unit price ×whole sale ratio).
    On "Price Tags" function, select whether to use the function to output records with the number of issues "0".
    Select whether to issue from price tag page or print using print tool.
    Select whether to use packing list switching function (set to use this when switching purchase statement by customers).
    Specify category of order of print-out.
    Enter whether to round down, round up or round calculation result (retail unit price ×whole sale ratio).
    Enter the number of spare items to be used in the HT Package size registration function (0 to 5).
    Please select the source of the Stock Qty to be set in the Order Basis Stock Qty`
    ...
    22
    Use
    Include Order Point (Lower Limit) Value
    20
    Set Item Info 002
    0
    Make all hidden.
    ...
    Screen
    Not use
    Order of Shipment No.
    Round-down
    0
    Possible Shipment Qty`

    let arrContent = settingContents.split("\n");
    let arrValue = values.split("\n");

    let tableBody = document.getElementsByClassName("body2");
    let childs = Array.from(tableBody[0].childNodes).filter((e) => e.className?.includes("row"));
    childs.map((child, index) => {
        child.childNodes[3].innerHTML = `<div class="enter-number-of-rows-to-be-added-when-adding-rows-is-selected">` + arrContent[index] + `</div>`
        child.childNodes[5].innerHTML = `<div class="_10">` + arrValue[index] + `</div>`
    })
}
...
var coll = Array.from(document.getElementsByClassName("header2"))
            ...
            .concat(Array.from(document.getElementsByClassName("header5")))
var i;
...
coll[i].addEventListener("click", function() {
    ...
    if (content.className.includes("displayNone")) {
        ...
        if(this.className.includes("header2")){
        ...
        }else if(this.className.includes("header3")){
        ...
        }else if(this.className.includes("header6")){
        ...
        }else if(this.className.includes("header8")){
            icon.innerHTML = `<path fill-rule="evenodd" clip-rule="evenodd" fill="white" d="M5 8C5 7.60218 5.06585 7.22064 5.18306 6.93934C5.30027 6.65804 5.45924 6.5 5.625 6.5H19.375C19.5408 6.5 19.6997 6.65804 19.8169 6.93934C19.9342 7.22064 20 7.60218 20 8C20 8.39782 19.9342 8.77936 19.8169 9.06066C19.6997 9.34197 19.5408 9.5 19.375 9.5H5.625C5.45924 9.5 5.30027 9.34197 5.18306 9.06066C5.06585 8.77936 5 8.39782 5 8Z"/>`
        }else if(this.className.includes("header5")){
            icon.innerHTML = `<path fill-rule="evenodd" clip-rule="evenodd" fill="white" d="M5 8C5 7.60218 5.06585 7.22064 5.18306 6.93934C5.30027 6.65804 5.45924 6.5 5.625 6.5H19.375C19.5408 6.5 19.6997 6.65804 19.8169 6.93934C19.9342 7.22064 20 7.60218 20 8C20 8.39782 19.9342 8.77936 19.8169 9.06066C19.6997 9.34197 19.5408 9.5 19.375 9.5H5.625C5.45924 9.5 5.30027 9.34197 5.18306 9.06066C5.06585 8.77936 5 8.39782 5 8Z"/>`
        }
    } else {
