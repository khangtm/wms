...
function createInitDataTable(){
    let tableBody = document.getElementsByClassName("body3");
    let childs = Array.from(tableBody[0].childNodes).filter((e) => e.className?.includes("row6"));
    childs.map((child, index) => {
        child.childNodes[7].innerHTML = `<div class="system-exe">ID_` + (index + 1) + `</div>`
        child.childNodes[9].innerHTML = `<div class="text-container7"><div class="system-exe">User` + (index + 1) + `</div></div>`
        child.childNodes[15].innerHTML = `<div class="_5370">537` + (index + 1) + `</div>`
    })
...
coll[i].addEventListener("click", function() {
    ...
    if (content.className.includes("displayNone")) {
        ...
        }else if(this.className.includes("header6") || this.className.includes("header5")){
