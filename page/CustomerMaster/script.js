...
function createInitDataTable(){
    let tableBody = document.getElementsByClassName("body5");
    let childs = Array.from(tableBody[0].childNodes).filter((e) => e.className?.includes("row4"));
    childs.map((child, index) => {
        child.childNodes[5].innerHTML = `<div class="cusdemo-001">CUSDEMO00` + (index + 1) + `</div>`
        child.childNodes[7].innerHTML = `<div class="cusdemo-001">CUSDEMO00` + (index + 1) + `</div>`
    })
}

var coll = Array.from(document.getElementsByClassName("header2"))
            .concat(Array.from(document.getElementsByClassName("header3")))
            .concat(Array.from(document.getElementsByClassName("header4")))
...
coll[i].addEventListener("click", function() {
    ...
    if (content.className.includes("displayNone")) {
        ...
        if(this.className.includes("header2")){
        ...
        }else if(this.className.includes("header4") || this.className.includes("header5") || this.className.includes("header7")){
            ...
            icon.innerHTML = `<path fill-rule="evenodd" clip-rule="evenodd" fill="black" d="M5 7.5C5 7.10218 5.06585 6.72064 5.18306 6.43934C5.30027 6.15804 5.45924 6 5.625 6H19.375C19.5408 6 19.6997 6.15804 19.8169 6.43934C19.9342 6.72064 20 7.10218 20 7.5C20 7.89782 19.9342 8.27936 19.8169 8.56066C19.6997 8.84197 19.5408 9 19.375 9H5.625C5.45924 9 5.30027 8.84197 5.18306 8.56066C5.06585 8.27936 5 7.89782 5 7.5Z"/>`
        }else if(this.className.includes("header8") || this.className.includes("header3")){
